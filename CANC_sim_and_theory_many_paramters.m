% Comparison of Careful Active Noise Control (CANC) algorithm
% simulation and theory with many different parameters and setups.
%
% For the paper:
...
N_simulations = 2;           % number of simulations in subset
%N_simulations = 10;           % number of simulations in subset
sim_set1 = 0;
sim_set2 = 0;
max_rate1 = nan*zeros(1,N_simulations);
max_rate2 = nan*zeros(1,N_simulations);
max_rate_T1 = nan*zeros(1,N_simulations);
max_rate_T2 = nan*zeros(1,N_simulations);
rng(637373);
...
L = 16*M;                      % simulation length (samples)
...
%for qv0 = 10.^(1:-0.02:-1.5)
for qv0 = 10.^(1:-0.2:-1.5)
...
for simulation=1:N_simulations
    ...
    residual_noise_power_i = mean(log_e(round(end-L/10):end, simulation).^2);
    min_residual_noise_power_i = qv0;
    excess_noise_power_i = mean(log_ex(round(end-L/10):end, simulation).^2);
...
n = 1:L;
cool_fig(1);
plot_xy_p3((1:L)/fs, 10*log10(smooth(log_ex.^2,Nw)));
grid on;
xlabel('time (s)');
...
sw(1:anc_on/Nc,:) = nan;
plot_xy_p3((1:Nc:L)/fs, 10*log10(sw));
grid on;
xlabel('time (s)');
...
max_rate = max(gamma(round(end/2):round(end*3/4),:));
varBeta = k*M*qu/qv0/(6*eta2);
max_rate_T = varBeta*(qp + abs(2*pF).^2);

if M == 8*Nw
   sim_set1 = sim_set1 + 1;
   max_rate1(sim_set1,:) = max_rate;
   max_rate_T1(sim_set1) = max_rate_T;
else
   sim_set2 = sim_set2 + 1;
   max_rate2(sim_set2,:) = max_rate;
   max_rate_T2(sim_set2) = max_rate_T;
end
...
cool_fig(11);
plot(10*log10(max_rate_T1), 10*log10(mean(max_rate1, 2)), 'o');
hold on;
plot(10*log10(max_rate_T2), 10*log10(mean(max_rate2, 2)), 'o');
hold off;
grid on;
xlabel('max theoretical rate (dB)');
ylabel('max rate (dB)');
set(gcf, 'name', 'max rate');
set(gcf, 'NumberTitle', 'off');
%save2pdf('max_rate', 11, 600);
